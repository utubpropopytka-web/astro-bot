...
async def handle_relocate(request):
    try:
        ...
        prompt = f"""Ты опытный иммиграционный консультант. Подготовь ориентировочный гайд по переезду.

Гражданин страны: {from_country}
Хочет переехать жить в город: {to_city}, страна: {to_country}

Используй веб‑поиск, чтобы найти актуальную информацию, и структурируй ответ так:

1. ВИЗОВЫЙ СТАТУС — в первом же абзаце чётко укажи: нужна ли гражданину {from_country} виза для краткосрочного (туристического) въезда в {to_country}, и на какой срок можно находиться без визы, если безвизовый режим есть.

2. ПУТИ ДЛЯ ПЕРЕЕЗДА — перечисли 2-4 реалистичных легальных способа переехать туда жить (рабочая виза, виза для фрилансеров/цифровых кочевников если она существует для этой страны, учебная виза, воссоединение семьи, виза предпринимателя/инвестора) — указывай только то, что реально существует именно для этой пары стран.

3. ДОКУМЕНТЫ — какие документы обычно требуются для самого реалистичного из перечисленных путей.

4. СЛЕДУЮЩИЕ ШАГИ — куда обращаться: посольство/консульство {to_country} в {from_country}, официальный иммиграционный портал и т.п.

Пиши кратко, по делу, раздели блоки переносами строк. Обязательно используй данные, найденные через поиск, а не только свои знания. Если что-то неточно или часто меняется — прямо об этом скажи. Длина: 350-500 слов."""
        ...
        guide_parts = [block.text for block in message.content if getattr(block, "type", None) == "text"]
        guide = "\n\n".join(guide_parts).strip()
        if not guide:
            guide = "Не удалось собрать гайд по переезду. Попробуй ещё раз."
