...
def get_plan_from_tribute_message(text: str):
    ...
    # Пакеты
    if 'всё в relo' in text_lower or 'все разделы' in text_lower or '1990' in text or '1 990' in text:
        return 'tariff_all'
    if 'куда и как переехать' in text_lower or '1490' in text or '1 490' in text:
        return 'tariff_top20guides'
    if 'куда уехать' in text_lower or 'топ 20' in text_lower or '790' in text:
        return 'tariff_top20'
    # Отдельные разовые покупки
    if '15 ответ' in text_lower or 'узнать больше о себе' in text_lower or '590' in text:
        return 'tariff_questions'
    # Гайд по одному городу — 690 ₽ разово (совпадает по сумме с подпиской 690 ₽/мес,
    # поэтому отличаем по словам "месяц"/"подписк" в тексте от Tribute)
    is_subscription_text = 'месяц' in text_lower or 'подписк' in text_lower
    if '1 город' in text_lower or ('переезд' in text_lower and not is_subscription_text) or ('690' in text and not is_subscription_text):
        return 1
    return 1
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✦ *RELO*\n\n"
        "💬 Узнай лучшие места на Земле для жизни, карьеры и любви по дате рождения — твой ТОП 20 городов\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_from_tribute:
        ...
        if plan == 'tariff_all':
            plan_label = 'Всё в RELO'
        elif plan == 'tariff_top20guides':
            plan_label = 'Куда и как переехать'
        elif plan == 'tariff_top20':
            plan_label = 'Куда уехать — ТОП 20 городов'
        elif plan == 'tariff_questions':
            plan_label = '15 ответов'
        else:
            plan_label = {1: 'Переезд по одному городу'}.get(plan, str(plan))
...
async def handle_create_payment(request):
    ...
    try:
        ...
        plan = data.get('plan')          # план: 1, 'tariff_top20', 'tariff_top20guides', 'tariff_all', 'tariff_questions'
