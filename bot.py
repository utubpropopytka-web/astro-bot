...
async def handle_question(request):
    try:
        ...
        prompt = f"""Ты профессиональный астролог. Дай глубокий персональный ответ на один конкретный вопрос по натальной карте.
...
Суть вопроса: {question_hint}

Напиши персональный астрологический ответ на этот конкретный вопрос. Используй реальные планеты из натальной карты клиента — упоминай конкретные знаки и их влияние именно на эту тему.

Структура ответа:
1. Главный вывод (2-3 предложения — прямой ответ на вопрос)
2. Астрологическое объяснение (какие именно планеты и знаки формируют этот паттерн, 2-3 предложения)
3. Практический совет — что конкретно делать с этим знанием (2-3 предложения)

Пиши тепло, на «ты». Будь конкретным — не общими фразами, а именно про эту натальную карту. Длина: 150-200 слов."""
