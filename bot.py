...
def get_plan_from_tribute_message(text: str):
    ...
    # Гайд по одному городу (390 ₽ разово — не путать с подпиской 390 ₽/мес)
    is_subscription_text = 'месяц' in text_lower or 'подписк' in text_lower
    if '1 город' in text_lower or ('переезд' in text_lower and not is_subscription_text) or ('390' in text and not is_subscription_text):
        return 1
    return 1
