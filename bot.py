...
Астрокарта показала следующий персональный Топ-5 городов для этого человека:
{top_cities}
...
2. Топ-3 города для релокации — возьми первые 3 города из списка выше и подробно объясни почему именно они подходят. Укажи конкретные планеты и зоны (AC/MC/DC/IC).
