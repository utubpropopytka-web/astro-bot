...
def get_plan_from_tribute_message(text: str) -> int:
    """Определяет тариф по тексту сообщения от Tribute"""
    text_lower = text.lower()
    if '5 город' in text_lower or '3000' in text or '3 000' in text:
        return 5
    elif '3 город' in text_lower or '1300' in text or '1 300' in text:
        return 3
    elif '1 город' in text_lower or '700' in text:
        return 1
    # Попробуем по названию товара
    if 'пять' in text_lower or '5' in text_lower:
        return 5
    elif 'три' in text_lower or '3' in text_lower:
        return 3
    elif 'один' in text_lower or '1' in text_lower:
        return 1
    return 0
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_from_tribute:
        ...
        if plan == 0:
            # Не смогли определить — выдаём базовый (1 город)
            plan = 1

        plan_label = {1: '1 город', 3: '3 города', 5: '5 городов'}.get(plan, str(plan))
        code = generate_access_code(plan)
        ...
        await msg.reply_text(
            ...
            f"Твой код доступа:\n"
            f"`{code}`\n\n"
            f"Скопируй этот код и введи его в приложении RELO во вкладке *Тарифы → Промокод*\n\n"
            f"⚠️ Код одноразовый — не передавай его другим.",
            parse_mode="Markdown"
        )
        logger.info(f"Access code {code} generated for plan {plan}, user {msg.from_user.id}")
        return
